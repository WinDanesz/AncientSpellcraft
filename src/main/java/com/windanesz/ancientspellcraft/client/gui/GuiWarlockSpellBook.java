...
public class GuiWarlockSpellBook extends GuiSpellInfo {
	...
	protected void drawForegroundLayer(int left, int top, int mouseX, int mouseY) {
		...
		if (discovered) {
			...
			String spellType = spell instanceof MetaSpellBuff ? I18n.format("spelltype.metamagic") :
					spell instanceof Contingency ? I18n.format("spelltype.contingency") : spell.getType().getDisplayName();

			this.fontRenderer.drawString(spellType, left + 17, top + 26, 0x777777);
		} else {
		...
		String element = I18n.format("gui.ebwizardry:spell_book.element", TextFormatting.RED + I18n.format("gui.ancientspellcraft:element.warlock"));
		if (!discovered)
			element = I18n.format(TRANSLATION_KEY_PREFIX + ".element_undiscovered");
		...
		if (getSpell().isContinuous)
			manaCost = I18n.format(TRANSLATION_KEY_PREFIX + ".mana_cost_continuous", getSpell().getCost());
		if (!discovered)
			manaCost = I18n.format(TRANSLATION_KEY_PREFIX + ".mana_cost_undiscovered");
