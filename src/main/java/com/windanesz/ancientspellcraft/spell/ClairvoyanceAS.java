...
public class ClairvoyanceAS extends Spell implements IOverrideSpell {
	...
	public ClairvoyanceAS() {
		super(Wizardry.MODID, "clairvoyance", SpellActions.POINT_UP, false);
		addProperties(RANGE, DURATION);
		WizardData.registerStoredVariables(LOCATION_KEY, DIMENSION_KEY);

		handleNetworkIDChange(this, 119);
	...
	public boolean cast(World world, EntityPlayer caster, EnumHand hand, int ticksInUse, SpellModifiers modifiers) {
		...
		// Monster Charm logic
		if (ItemArtefact.isArtefactActive(caster, ASItems.charm_monster_charm)) {
			List<ItemStack> charm = ASBaublesIntegration.getEquippedArtefactStacks(caster, ItemArtefact.Type.CHARM);

 				if (!charm.isEmpty() && charm.get(0).getItem() == ASItems.charm_monster_charm) {
				...
				if (mobId != null) {
					...
					double radius = getProperty(RANGE).floatValue() * 0.25 * modifiers.get(WizardryItems.range_upgrade);
					...
					if (closest != null) {
						...
						if (data != null) {
							data.setVariable(LOCATION_KEY, targetPos);
							data.setVariable(DIMENSION_KEY, caster.dimension);
							if (!world.isRemote) {
		...
		if (data != null && !caster.isSneaking()) {
			...
			if (dimension != null && caster.dimension == dimension) {
				if (location != null) {
					...
					if (!world.isRemote)
						caster.sendStatusMessage(new TextComponentTranslation("spell." + this.getUnlocalisedName() + ".searching"), true);
					...
					if (!world.isRemote)
						caster.sendStatusMessage(new TextComponentTranslation("spell." + this.getUnlocalisedName() + ".outofrange"), true);
	...
	public static void onRightClickBlockEvent(PlayerInteractEvent.RightClickBlock event) {
		...
		if (event.getEntityPlayer().isSneaking()) {
			...
			if (stack.getItem() instanceof ISpellCastingItem
					&& ((ISpellCastingItem) stack.getItem()).getCurrentSpell(stack) instanceof ClairvoyanceAS) {
