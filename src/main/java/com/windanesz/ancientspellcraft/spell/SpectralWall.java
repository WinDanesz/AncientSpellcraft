...
public class SpectralWall extends SpellRay implements IClassSpell {

	private static final String BLOCK_LIFETIME = "block_lifetime";

	public SpectralWall() {
		super(AncientSpellcraft.MODID, "spectral_wall", SpellActions.SUMMON, false);
	...
	protected boolean onBlockHit(World world, BlockPos pos, EnumFacing side, Vec3d hit, EntityLivingBase caster, Vec3d origin, int ticksInUse, SpellModifiers modifiers) {
		...
		if (caster.isSneaking() && world.getBlockState(pos).getBlock() == ASBlocks.CONJURED_MAGMA) {
		...
		if (!world.isRemote) {
			...
			for (int i = 0; i < 3 + blastUpgradeCount; i++) {
					...
					for (int j = 2; j < 3  +blastUpgradeCount; j++) {
						wall.add(new BlockPos(pos.getX(), pos.getY(), pos.getZ()).offset(direction, i).offset(EnumFacing.UP, j));
						wall.add(new BlockPos(pos.getX(), pos.getY(), pos.getZ()).offset(direction.getOpposite(), i).offset(EnumFacing.UP, j));
					}
			}
			...
			builder.blockLifetime = 600;//(int) ((getProperty(EFFECT_DURATION).floatValue() * modifiers.get(WizardryItems.duration_upgrade)));
			...
			builder.setBlockToBuild(WizardryBlocks.spectral_block.getDefaultState());
