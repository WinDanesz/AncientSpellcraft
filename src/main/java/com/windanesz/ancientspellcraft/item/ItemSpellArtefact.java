...
public class ItemSpellArtefact extends ItemASArtefact implements ISpellCastingItem, IManaStoringItem, ITickableArtefact {
	...
	public ItemSpellArtefact(EnumRarity rarity, Type type, int mana) {
		super(rarity, type);
		this.setMaxDamage(mana);
	}

	@Override
	@SideOnly(Side.CLIENT)
	public boolean hasEffect(ItemStack stack) {
		// only has effect if it's an epic artefact and if it has charges
		return getForgeRarity(stack) == EnumRarity.EPIC && !isManaEmpty(stack);
	}
