...
public class ItemAmuletOfResistance extends ItemASArtefact implements IManaStoringItem, IWorkbenchItem, ITickableArtefact {
    ...
    public ItemAmuletOfResistance() {
        super(EnumRarity.RARE, ItemArtefact.Type.AMULET);
    ...
    private boolean imbueWithPotion(ItemStack amulet, ItemStack potionStack, EntityPlayer player) {
        ...
        if (!effects.isEmpty()) {
            ...
            player.sendMessage(new TextComponentTranslation("item.ancientspellcraft:amulet_of_resistance.imbued", 
                new TextComponentTranslation(potion.getName()).getFormattedText()));
            return true;
        }
        return false;
    }
