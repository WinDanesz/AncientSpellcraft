...
public class ItemVerdantCrucible extends ItemDailyArtefact {
	...
	public void performAction(EntityPlayer player) {
		if (!player.world.isRemote && !ingredientEntries.isEmpty()) {
			IngredientEntry entry = ingredientEntries.get(player.getRNG().nextInt(ingredientEntries.size()));
			...
			if (item != null) {
				...
				int retries = 10;
				while (retries-- > 0) {
					stack = new ItemStack(item, count, entry.meta);
