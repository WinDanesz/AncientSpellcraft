...
/**
 * This is almost the same as the spell {@link electroblob.wizardry.constants.Tier} tiers, with some modifications and higher tiers.
 * @since AncientSpellcraft 1.4
 * @author Electroblob (from the Tier class), WinDanesz
 */
public enum RitualTier {
	...
	/** + some kick-ass higher tiers in the future **/
	...
	RitualTier(int maxCharge, int upgradeLimit, int weight, Style colour, String name){
	...
	/** Returns the tier with the given name, or throws an {@link IllegalArgumentException} if no such
	 * tier exists. */
	public static RitualTier fromName(String name){

		for(RitualTier tier : values()){
	 ...
	 */
	public static RitualTier getWeightedRandomTier(Random random, RitualTier... tiers){

		if(tiers.length == 0) tiers = values();

		int totalWeight = 0;

		for(RitualTier tier : tiers) totalWeight += tier.weight;

		int randomiser = random.nextInt(totalWeight);
		int cumulativeWeight = 0;

		for(RitualTier tier : tiers){
