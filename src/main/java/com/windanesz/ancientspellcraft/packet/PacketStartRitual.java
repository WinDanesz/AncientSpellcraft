 ...
 */
public class PacketStartRitual implements IMessageHandler<PacketStartRitual.Message, IMessage> {

	@Override
	public IMessage onMessage(PacketStartRitual.Message message, MessageContext ctx){

		// Just to make sure that the side is correct
		if(ctx.side.isClient()){
			// Using a fully qualified name is a good course of action here; we don't really want to clutter the proxy
			// methods any more than necessary.
			net.minecraft.client.Minecraft.getMinecraft().addScheduledTask(() -> AncientSpellcraft.proxy.handleRitualStartPacket(message));
		}
