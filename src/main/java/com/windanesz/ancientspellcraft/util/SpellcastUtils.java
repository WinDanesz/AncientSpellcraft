...
public class SpellcastUtils {
	...
	public static boolean tryCastSpellAsMob(EntityLiving entityLiving, Spell spell, @Nullable EntityLivingBase target) {
		...
		if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Pre(SpellCastEvent.Source.WAND, spell, entityLiving, new SpellModifiers()))) {return false;}
	...
	public static boolean tryCastSpellAsPlayer(EntityPlayer player, Spell spell, EnumHand hand,
		...
		// if true, the spell cast fails
		if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Pre(source, spell, player, modifiers))) {return false;}
	...
	public static boolean tryCastSpellAtLocation(World world, BlockPos pos, EnumFacing direction, Spell spell, SpellModifiers modifiers) {
		...
		if (!spell.canBeCastBy(new TileEntityDispenser())) {return false;}
	...
	public static boolean proxyTargetedSpell(World world, EntityLivingBase caster, @Nullable Entity t, Spell spell, SpellModifiers modifiers) {
		...
		if (t instanceof EntityLivingBase) {
			...
			EntitySkeletonMageMinion mage = new EntitySkeletonMageMinion(world);
